...
def alumno_clases(page: ft.Page, alumno_id: int):
    ...
    def registrar_asistencia(e, clase_id: int, file_picker: ft.FilePicker):
        def seleccionar_archivo(e: ft.FilePickerResultEvent):
            ...
            try:
                ...
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO asistencias (alumno_id, clase_id, fecha_hora, evidencia_path)
                    VALUES (?, ?, ?, ?)
                """, (alumno_id, clase_id, datetime.now().isoformat(), ruta_destino))
                conn.commit()
                conn.close()
    ...
    def cargar_datos():
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE alumno_id = ? ORDER BY fecha_hora DESC
            """, (alumno_id,))
